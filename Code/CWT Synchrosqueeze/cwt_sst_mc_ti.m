function [Ws Rs Ts aj] = cwt_sst_mc_ti(s,gamma,mu,ci,nv,ti)
% computes the CWT Ws, the phase transform of CWT Rs, and the synchrosqueezing transform SST Ts of the signal s.
%
% Input:
%   s: signal
%   gamma: threshold
...
for ai = 1:na
    ...
    dxcpsi = F1*((dpsih .* xh))';
    dWs(ai) = conj(dxcpsi);  % conj, different from %dxcpsi = (ifft(dpsih .* xh));
end
...
for wi = 2:n/2
    w = wl(wi);
    tmp1 = find(imag(Rs)>w-dw/2 & imag(Rs)<=w+dw/2);
    if length(tmp1)>=1
        Ts(wi) = Ts(wi) + log(2)/nv*ones(1,length(tmp1))*Ws(tmp1);  
    end
end
end

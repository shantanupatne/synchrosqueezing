%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   
%   Weak linear chirp detection through the adaptive Liènard oscillator
...
format long;
%close all;
load Data/ChirpCuadratica.mat Chirp  %Read chirp signal
...
xl=0;   %Initial Conditions
yl=0;
%%  Chirp Parameters

Wini=5e9;   %Initial Frequency
fini=Wini/(2*pi);
ffin=(15e14/pi)*(b-a) + fini;
K=(ffin-fini)/(b-a);    %Linear rate frequency
W=Wini;
...
global ts   %Step size
ts=h;
NVent=length(TV);   %Number of windows
...
for MC=1:1:MonteCarlo
    ...
    for Z=2:1:NVent-1
            ...
            [deltaT1]= Delta3R(Umbral,xc1');    
            [deltaT2]= Delta3R(Umbral,xc2');     
            [deltaT3]= Delta3R(Umbral,xc3');     
            [deltaT4]= Delta3R(Umbral,xc4');     
            [deltaT5]= Delta3R(Umbral,xc5');              
...
hold on;
for s=1:1:MC   
    stairs(Tfreq,freq(s,:),'g');
end

stairs(Tfreq,frecMonteCarlo,'r',LineWidth=2);
%Plot real chirp
ChirpIdeal = 0.1*((5e9/(2*pi)) +  2*(15e14/2*pi)*(Tfreq-0.2*tv) - 3*(98e19/2*pi)*(Tfreq-0.2*tv).^2 ) + 0.9035*(5e9/(2*pi));
%plot estimated chirp
...
% fig1 = openfig('/results/LinearChirp.fig');
% filename1 = '/results/LinearChirp.png';
% saveas(fig1, filename1);
% fig2 = openfig('/results/QuadraticChirp.fig');
% filename2 = '/results/QuadraticChirp.png';
